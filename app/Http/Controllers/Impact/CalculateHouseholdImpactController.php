...
class CalculateHouseholdImpactController extends Controller
{
    public function __invoke(CalculateHouseholdImpactRequest $request)
    {
        ...
        $energyUsage = $request->input('energyUsage') ?? 3100 * ($occupants / 2); // in KWh (source: https://www.energy-uk.org.uk/energy-industry/watt-powers.html)
        $gasUsage = $request->input('gasUsage') ?? 12000 * ($occupants / 2); // in KWh (source: https://selectra.co.uk/energy/guides/consumption/average-consumption-uk)
